...
if CCXTversion < fixVersionFormat(minCCXTversion):
    ...
    print(
        "While it may run with earlier versions wrong behaviors are expected to happen."
    )
...
SHOW_BREAKEVEN = True  # in positions when availab le
...
UPDATE_ORDERS_FREQUENCY = (
    0.25  # frametime in seconds at which the orders queue is refreshed.
)
LOGS_DIRECTORY = "logs"
MARGIN_MODE_NONE = "------"
...
def writeConfig():
    with open("config.json", "w") as f:
        ...
        configString += (
            '\t\t"REFRESH_POSITIONS_FREQUENCY":'
            + str(REFRESH_POSITIONS_FREQUENCY)
            + ",\n"
        )
        configString += (
            '\t\t"UPDATE_ORDERS_FREQUENCY":' + str(UPDATE_ORDERS_FREQUENCY) + ",\n"
        )
        ...
        configString += (
            '\t\t"SHOW_LIQUIDATION":' + str(SHOW_LIQUIDATION).lower() + ",\n"
        )
...
class position_c:
    ...
    def generatePrintString(self) -> str:
        ...
        unrealizedPnl = (
            0
            if (self.getKey("unrealizedPnl") == None)
            else float(self.getKey("unrealizedPnl"))
        )
        initialMargin = (
            0
            if (self.getKey("initialMargin") == None)
            else float(self.getKey("initialMargin"))
        )
        collateral = (
            0.0
            if (self.getKey("collateral") == None)
            else float(self.getKey("collateral"))
        )
        if initialMargin != 0:
        ...
        positionModeChar = (
            "[H]" if (self.thisMarket["local"]["positionMode"] == "hedged") else ""
        )
        levStr = (
            "?x"
            if (self.thisMarket["local"]["leverage"] == 0)
            else str(self.thisMarket["local"]["leverage"]) + "x"
        )
        ...
        if initialMargin != 0:
            string += " * " + "{:.4f}[$]".format(initialMargin)
        elif collateral != 0:
        ...
        if self.getKey("liquidationPrice") != None and SHOW_LIQUIDATION:
            string += " * " + "[li]{:.3f}".format(
                float(self.getKey("liquidationPrice"))
            )
...
class order_c:
    def __init__(
        self, symbol="", side="", quantity=0.0, leverage=1, delay=0, reduceOnly=False
    ) -> None:
        self.symbol = symbol
        self.type = "market"
        self.side = side
...
class account_c:
    def __init__(
        ...
        self.MARGIN_MODE = (
            "cross"
            if (marginMode != None and marginMode.lower() == "cross")
            else "isolated"
        )
        ...
        for key in marketKeys:
            ...
            if thisMarket.get("contractSize") == None:
                ...
                if self.exchange.id == "phemex":
                    ...
                    s = description[
                        description.find("Each contract is worth")
                        + len("Each contract is worth ") :
                    ]
                    list = s.split(" ", 1)
                    cs = float(list[0])
                    if cs != 1.0:
                    ...
                    print(
                        "WARNING: Market", self.exchange.id, "doesn't have contractSize"
                    )
            ...
            try:
                ...
                raise ValueError(
                    "Market", self.exchange.id, "doesn't have precision value"
                )
    ...
    def print(
        self, *args, sep=" ", **kwargs
    ):  # adds account and exchange information to the message
        print(
            timeNow(),
            "["
            + self.accountName
        ...
        self.logger.info(
            "[" + dateString() + "][" + timeNow() + "] " + sep.join(map(str, args)),
            **kwargs,
        )
    ...
    def verifyLeverageRange(self, symbol, leverage) -> int:
        ...
        if maxLeverage != None and maxLeverage < leverage:
            self.print(
                " * WARNING: Leverage out of bounds. Readjusting to",
                str(maxLeverage) + "x",
            )
    ...
    def updateSymbolPositionMode(self, symbol):
        ...
        if (
            self.exchange.has.get("setPositionMode") != True
            and self.markets[symbol]["local"]["positionMode"] != "oneway"
        ):
            self.print(
                " * E: updateSymbolPositionMode: Exchange",
                self.exchange.id,
                "doesn't have setPositionMode nor is set to oneway",
            )
            return

        if (
            self.markets[symbol]["local"]["positionMode"] != "oneway"
            and self.exchange.has.get("setPositionMode") == True
        ):
            ...
            try:
                ...
                for a in e.args:
                    if (
                        ...
                        print(
                            " * E: updateSymbolLeverage->set_position_mode:", a, type(e)
                        )
            else:
                # was everything correct, tho?
                code = 0
                if (
                    self.exchange.id == "bybit"
                ):  # they didn't receive enough love as children
    ...
    def updateSymbolLeverage(self, symbol, leverage):
        ...
        if (
            ...
            and self.exchange.has.get("setMarginMode") == True
        ):
            params = {}
            ...
            try:
                response = self.exchange.set_margin_mode(
                    self.MARGIN_MODE, symbol, params
                )
                ...
                for a in e.args:
                    if (
                        '"retCode":140026' in a
                        or "No need to change margin type" in a
                        ...
                        print(
                            " * E: updateSymbolLeverage->set_margin_mode:", a, type(e)
                        )
            else:
                # was everything correct, tho?
                code = 0
        ...
        if (
            self.markets[symbol]["local"]["leverage"] != leverage
            and self.exchange.has.get("setLeverage") == True
        ):
            ...
            params = {}
            if (
                self.exchange.id == "coinex"
            ):  # coinex always updates leverage and marginMode at the same time
                params["marginMode"] = self.markets[symbol]["local"][
                    "marginMode"
                ]  # use current marginMode to avoid triggering an error
            elif self.exchange.id == "okx":
                ...
                if self.markets[symbol]["local"]["positionMode"] != "oneway":
                    response = self.exchange.set_leverage(
                        leverage, symbol, params={"side": "LONG"}
                    )
                    response2 = self.exchange.set_leverage(
                        leverage, symbol, params={"side": "SHORT"}
                    )
    ...
    def fetchBalance(self):
        ...
        if self.exchange.id == "krakenfutures":
            ...
            return {
                "free": float(data.get("availableMargin")),
                "used": float(data.get("initialMarginWithOrders")),
                "total": float(data.get("balanceValue")),
            }
    ...
    def findMaxLeverageForSymbol(self, symbol) -> float:
        ...
        if maxLeverage == None:
            ...
            if self.exchange.has["fetchLeverage"]:
                ...
                if (
                    info != None
                    and info.get("maxLongLeverage") != None
                    and info.get("maxShortLeverage") != None
                ):
                    maxLeverage = min(
                        int(info["maxLongLeverage"]), int(info["maxShortLeverage"])
                    )
    ...
    def contractsFromUSDT(self, symbol, amount, price, leverage=1.0) -> float:
        ...
        return (
            roundDownTick(coin, precision)
            if (coin > 0)
            else roundUpTick(coin, precision)
        )
    ...
    def refreshPositions(self, v=verbose):
        ...
        try:
            ...
            positions = self.exchange.fetch_positions(
                symbols, params={"settle": self.SETTLE_COIN}
            )  # the 'settle' param is only required by phemex

        except Exception as e:
            a = e.args[0]
            if (
                "OK" in a
            ):  # Coinex raises an exception to give an OK message when there are no positions... don't look at me, look at them
                positions = []
            elif (
                isinstance(e, ccxt.OnMaintenance)
                or isinstance(e, ccxt.NetworkError)
                or isinstance(e, ccxt.RateLimitExceeded)
                or isinstance(e, ccxt.RequestTimeout)
                or isinstance(e, ccxt.ExchangeNotAvailable)
                or "not available" in a
            ):
                failed = True

                if (
                    "Remote end closed connection" in a
        ...
        if failed:
            ...
            if self.refreshPositionsFailed == 10:
                print(
                    timeNow(),
                    self.exchange.id,
                    "* W: Refreshpositions has failed 10 times in a row",
                )
            return
        ...
        if self.refreshPositionsFailed >= 10:
            print(
                timeNow(),
                self.exchange.id,
                "* W: Refreshpositions has returned to activity",
            )
        ...
        for thisPosition in positions:
            ...
            if self.exchange.id == "bybit":
                thisPosition["hedged"] = (
                    True if (thisPosition["info"].get("positionIdx") != "0") else False
                )
            ...
            if (
                thisPosition.get("hedged") != None
            ):  # None means the exchange only supports oneWay
                self.markets[symbol]["local"]["positionMode"] = (
                    "hedged" if (thisPosition.get("hedged") == True) else "oneway"
                )
            ...
            if thisPosition.get("marginMode") == None:
                if (
                    self.exchange.id == "bybit"
                ):  # tradeMode - Classic & UTA (inverse): 0: cross-margin, 1: isolated margin
                    self.markets[symbol]["local"]["marginMode"] = (
                        "isolated"
                        if thisPosition["info"]["tradeMode"] == "1"
                        else "cross"
                    )
                    ...
                    print(
                        " * W: refreshPositions: Could not get marginMode for", symbol
                    )
                    thisPosition["marginMode"] = MARGIN_MODE_NONE
            else:
                self.markets[symbol]["local"]["marginMode"] = thisPosition.get(
                    "marginMode"
                )
            ...
            if thisPosition.get("leverage") != None:
                ...
                if leverage != thisPosition.get(
                    "leverage"
                ):  # kucoin sends weird fractional leverage. Ignore it
                    leverage = -1
                ...
                try:
                    ...
                    if self.exchange.id == "bitget":
                        if response["data"]["marginMode"] == "crossed":
                            ...
                            longLeverage = int(
                                response["data"].get("fixedLongLeverage")
                            )
                            shortLeverage = int(
                                response["data"].get("fixedShortLeverage")
                            )
            ...
            if leverage != -1:
                ...
                print(
                    " * W: refreshPositions: Couldn't find leverage for",
                    self.exchange.id,
                )

            self.positionslist.append(
                position_c(symbol, thisPosition, self.markets[symbol])
            )
    ...
    def fetchClosedOrderById(self, symbol, id):
        try:
            response = self.exchange.fetch_closed_orders(
                symbol, params={"settleCoin": self.SETTLE_COIN}
            )
        except Exception as e:
    ...
    def fetchOpenOrderById(self, symbol, id):
        try:
            response = self.exchange.fetch_open_orders(
                symbol, params={"settleCoin": self.SETTLE_COIN}
            )
        except Exception as e:
            # Exception: ccxt.base.errors.ExchangeError: phemex {"code":39999,"msg":"Please try again.","data":null}
    ...
    def removeFirstCompletedOrder(self):
        ...
        for order in self.activeOrders:
            if order.timedOut():
                self.print(
                    " * E: Active Order Timed out",
                    order.symbol,
                    order.side,
                    order.quantity,
                    str(order.leverage) + "x",
                )
            ...
            if (
                self.exchange.id == "phemex"
                or self.exchange.id == "bybit"
                or self.exchange.id == "krakenfutures"
            ):
                if order.type == "limit":
                ...
                try:
                    ...
                    if (
                        isinstance(e, ccxt.InvalidOrder)
                        or "order not exists" in e.args[0]
                    ):
                        continue
            ...
            if (
                response == None
            ):  # FIXME: Check if this is really happening by printing it.
            ...
            if order.type == "limit":
                if self.exchange.id == "coinex":
                    response["clientOrderId"] = response["info"]["client_id"]  # HACK!!
                self.print(
                    " * Linmit order placed:",
                    order.symbol,
                    order.side,
            ...
            if remaining > 0 and (status == "canceled" or status == "closed"):
                print("r...", end="")
                self.ordersQueue.append(
                    order_c(order.symbol, order.side, remaining, order.leverage, 0.5)
                )
                self.activeOrders.remove(order)
                return True
    ...
    def cancelLimitOrder(self, symbol, customID) -> bool:
        ...
        if (
            self.exchange.id == "krakenfutures"
            or self.exchange.id == "kucoinfutures"
            or self.exchange.id == "coinex"
            or self.exchange.id == "bitget"
        ):
            ...
            try:
                ...
                self.print(
                    " * E: Unhandled exception in cancelLimitOrder:", e.args[0], type(e)
                )
                return
            else:
                for o in response:
                    if (
                        (
                            o["info"].get("cliOrdId") != None
                            and o["info"]["cliOrdId"] == customID
                        )
                        or (
                            o["info"].get("client_id") != None
                            and o["info"]["client_id"] == customID
                        )
        ...
        try:
            ...
            a = e.args[0]
            if (
                isinstance(e, ccxt.OrderNotFound)
                or isinstance(e, ccxt.BadRequest)
        ...
        for order in self.ordersQueue:
            ...
            if order.side == "changeleverage":
                if self.markets[order.symbol]["local"]["leverage"] == order.leverage:
                    self.print(
                        " * Leverage changed to",
                        self.markets[order.symbol]["local"]["leverage"],
                    )
                else:
            ...
            if order.reduceOnly:
                ...
                if (
                    self.exchange.id != "coinex"
                ):  # coinex interprets reduceOnly as being in hedge mode. Skip the problem by now
                    params["reduceOnly"] = True

            if (
                self.exchange.id == "kucoinfutures"
            ):  # Kucoin doesn't use setLeverage nor setMarginMode
            ...
            order.quantity = roundToTick(
                order.quantity, self.findPrecisionForSymbol(order.symbol)
            )
            ...
            try:
                ...
                if (
                    isinstance(e, ccxt.InsufficientFunds)
                    or '"code":"40762"' in a
                    or 'code":101204' in a
                    or '"code":-4131' in a
                    or 'code":101253' in a
                    ...
                    if not order.reduced:
                        ...
                        price = (
                            self.fetchSellPrice(order.symbol)
                            if (type == "sell")
                            else self.fetchBuyPrice(order.symbol)
                        )
                        available = self.fetchAvailableBalance() * 0.985
                        order.quantity = self.contractsFromUSDT(
                            order.symbol, available, price, order.leverage
                        )
                        order.reduced = True
                        if order.quantity < self.findMinimumAmountForSymbol(
                            order.symbol
                        ):
                            self.print(
                                " * E: Balance insufficient: Minimum contracts required:",
                                self.findMinimumAmountForSymbol(order.symbol),
                                " Cancelling",
                            )
                            self.ordersQueue.remove(order)
                        else:
                            self.print(
                                " * E: Balance insufficient: Was",
                                oldQuantity,
                                "Reducing to",
                                order.quantity,
                                "contracts",
                        ...
                        if order.quantity < 20 and precision >= 1:
                            self.print(
                                " * E: Balance insufficient: Reducing by one contract"
                            )
                            order.quantity -= precision
                        else:
                            order.quantity = roundDownTick(
                                order.quantity * 0.95, precision
                            )
                            if order.quantity < self.findMinimumAmountForSymbol(
                                order.symbol
                            ):
                ...
                if isinstance(e, ccxt.InvalidOrder):
                    ...
                    if "Order price is not within" in a:
                        ...
                        self.print(
                            " * E: Order size invalid:",
                            order.quantity,
                            "x" + str(order.leverage),
                        )
                        self.ordersQueue.remove(order)
                    elif (
                        '"retCode":20094' in a
                        or '"code":-4015' in a
                        or "ID already exists" in a
                    ):
                        self.print(
                            " * E: Cancelling Linmit order: ID [",
                            order.customID,
                            "] was used before",
                        )
                        self.ordersQueue.remove(order)
                ...
                if (
                    self.exchange.id == "bingx"
                    and order.type == "limit"
                    and '"code":101500' in a
                ):
                    self.print(
                        " * E: Cancelling Linmit order: ID [",
                        order.customID,
                ...
                if (
                    "Too Many Requests" in a
                    or "too many request" in a
                    or "service too busy" in a
                    or "system is busy" in a
                ):
                ...
                self.print(
                    " * E: UpdateOrdersQueue: Unhandled exception. Cancelling:",
                    a,
                    type(e),
                )
                self.ordersQueue.remove(order)
            ...
            if (
                remaining != None
                and remaining > 0
                and (status == "canceled" or status == "closed")
            ):
                print("r...", end="")
                self.ordersQueue.append(
                    order_c(order.symbol, order.side, remaining, order.leverage, 0.5)
                ...
                self.ordersQueue.remove(order)
                continue
            if (remaining == None or remaining == 0) and (
                response.get("status") == "closed" or response.get("status") == "filled"
            ):
                self.print(
                    " * Order successful:",
                    order.symbol,
    ...
    def proccessAlert(self, alert: dict):
            ...
            if (
                ...
                if alert.get("timestamp") + ALERT_TIMEOUT < time.monotonic():
                    ...
                    if isinstance(e, ccxt.RateLimitExceeded):
                        ...
                        self.print(
                            " * E: Couldn't reach the server: Retrying in 30 seconds",
                            e,
                            type(e),
                        )
        ...
        if self.exchange.id == "bybit" and (
            command == "position" or command == "close"
        ):
        ...
        if command == "changeleverage":
            ...
            if self.markets[symbol]["local"]["leverage"] == leverage:
                ...
                return
            self.ordersQueue.append(
                order_c(symbol, "changeleverage", leverage=leverage)
            )
            return
        ...
        if command == "position":
            if pos == None or pos.getKey("contracts") == None:
                ...
                self.markets[symbol]["local"]["marginMode"] != self.MARGIN_MODE
                and self.exchange.has["setMarginMode"]
            ):
                # to change marginMode we need to close the old position first
                if pos.getKey("side") == "long":
                    self.ordersQueue.append(
                        order_c(symbol, "sell", pos.getKey("contracts"), 0)
                    )
                else:
                    self.ordersQueue.append(
                        order_c(symbol, "buy", pos.getKey("contracts"), 0)
                    )
                ...
                if quantity < minOrder:
                    ...
                    if leverage != self.markets[symbol]["local"]["leverage"]:
                        ...
                        self.print(
                            " * Order completed: Request matched current position"
                        )
                    return
                # if we are reducing the size and changing leverage we want to reduce size first, then modify the leverage
                if (
                    command == "sell"
                    and leverage != self.markets[symbol]["local"]["leverage"]
                    and self.markets[symbol]["local"]["leverage"] != 0
                ):  # kucoin has 0 local leverage until an order is processed
                    alert = {
                        "symbol": symbol,
                    ...
                    leverage = self.markets[symbol]["local"][
                        "leverage"
                    ]  # reduce the position with current leverage
            # fall through

        if command == "buy" or command == "sell":
            # fetch available balance and price
            price = (
                self.fetchSellPrice(symbol)
                if (command == "sell")
                else self.fetchBuyPrice(symbol)
            )
            ...
            if pos != None:
                ...
                if not isLimit and (
                    (positionSide == "long" and command == "sell")
                    or (positionSide == "short" and command == "buy")
                ):
                    ...
                    if self.exchange.id == "bingx":
                        if quantity > positionContracts:
                            self.ordersQueue.append(
                                order_c(symbol, command, positionContracts, 0)
                            )
                            quantity -= positionContracts
                            self.ordersQueue.append(
                                order_c(symbol, command, quantity, leverage)
                            )
                            return

                        self.ordersQueue.append(
                            order_c(
                                symbol, command, quantity, leverage, reduceOnly=True
                            )
                        )
                        return
                    ...
                    if (
                        self.exchange.id == "bybit" or self.exchange.id == "okx"
                    ) and quantity > positionContracts:
                        self.ordersQueue.append(
                            order_c(
                                symbol, command, positionContracts, 0, reduceOnly=True
                            )
                        )
                        quantity -= positionContracts
                        if quantity > minOrder:
                    ...
                    if quantity >= canDoContracts + positionContracts:
                        ...
                        self.ordersQueue.append(
                            order_c(symbol, command, order1, leverage)
                        )
                        ...
                        if quantity >= minOrder:  # we are done (should never happen)
                            self.ordersQueue.append(
                                order_c(symbol, command, quantity, leverage, 1.0)
                            )
...
def updateOrdersQueue():
    for account in accounts:
        ...
        if (
            numOrders > 0
            and (len(account.ordersQueue) + len(account.activeOrders)) == 0
        ):
...
def parseAlert(data, account: account_c):
    ...
    for token in tokens:
        if (
            account.findSymbolFromPairName(token) != None
        ):  # GMXUSDTM, GMX/USDT:USDT and GMX/USDT are all acceptable formats
            ...
            token[:1] == "-"
        ):  # this is a minus symbol! What a bitch (value in base currency)
            ...
            alert["command"] = "buy"
            print(
                "WARNING: 'long' and 'short' commands are deprecated and will be removed in the future. Please use 'buy' and 'sell' instead"
            )
    ...
    if (
        alert["command"] == "buy"
        or alert["command"] == "sell"
        or alert["command"] == "position"
    ):
        ...
        if (
            alert["command"] == "sell" and alert["quantity"] < 0
        ):  # be flexible with sell having a negative amount
    ...
    if limitToken != None:
        if alert["command"] != "buy" and alert["command"] != "sell":
            return {
                "Error": " * E: Limit orders can only be used with buy/sell commands"
            }

        v = limitToken.split(":")
        if len(v) != 3:
            return {
                "Error": " * E: Limit command must be formatted as 'limit:customID:price' "
            }
        else:
            alert["customID"] = v[1]
    ...
    if cancelToken != None:
        ...
        if len(v) != 2:
            return {
                "Error": " * E: Cancel command must be formatted as 'cancel:customID' "
            }
        alert["customID"] = v[1]
    ...
    if alert["customID"] != None:
        if len(alert["customID"]) < 2 or len(alert["customID"]) > 30:
            return {
                "Error": " * E: customID must be longer than 2 characters and shorter than 30' "
            }
...
try:
    ...
    with open("accounts.json", "x") as f:
        ...
        f.close()
    print(
        "File 'accounts.json' not found. Template created. Please fill your API Keys into the file and try again"
    )
    print("Exiting.")
    raise SystemExit()
...
for ac in accounts_data:
    ...
    try:
        account = account_c(
            exchange, account_id, api_key, secret_key, password, marginMode, settleCoin
        )
    except Exception as e:
